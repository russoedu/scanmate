import type { InkProbe } from '../change-detection'
import { toGrayscale } from '@scanmate/ink'
import type { GrayImage, Raster, TextRun } from '@scanmate/ink'
import type { MatchOptions, OcrEngine, RecheckPass, Templates, TextDifference } from '@scanmate/ocr'
...
export async function settleDisputes (input: SettlementInput): Promise<Settlement[]> {
  ...
  // The reader is loaded when there is something to settle, not when this module is.
  const { collectTemplates, judgeRun, printPolarity, readRun, verifyPrintedRun } = await loadOcr()
  ...
  for (const [index, difference] of differences.entries()) {
    ...
    settlements.push({
      ...
      readings,
      steady:  read && unanimous(readings.original, rules, judgeRun) && unanimous(readings.scanned, rules, judgeRun),
    })
  }
...
function unanimous (readings: readonly string[], rules: MatchOptions, judgeRun: (a: string, b: string, rules: MatchOptions) => { agrees: boolean }): boolean {
