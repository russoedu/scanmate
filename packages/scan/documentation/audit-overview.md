...
`Scanmate.calibrate` in `@scanmate/scan` runs the whole corpus from file paths, one document at a time, each case carrying settings of its own where the corpus is not uniform. `audit.md` has the numbers from a calibration on 75 documents made from real scans, and what they say about scan quality.
